    ...
    <article class="span12 data-block nested">
        ...
        <div class="data-container">
            ...
            <section>
                ...
                <div id="sec2">
                    <h1>Manage Super Moderators</h1>
                    <?php
                        if(isset($_GET['add'])) {
                            if($_POST['submit']) {
                                $smod_user = $_POST['smod_user'];
                                $smod_pass = $_POST['smod_pass'];
                                mysql_query("INSERT INTO `supermod`(`id`, `username`, `password`, `hid`) VALUES (NULL, '".$smod_user."', '".$smod_pass."', '1')");
                                echo 'Super Moderator '.$smod_user.' added.';
                            } else {
                                ?>
                                    <form method="POST" action="?add=do">
                                        SMOD Username: <input type='text' name='smod_user' value=''><br>
                                        SMOD Password: <input type='password' name='smod_pass' value=''><br>
                                        <input type="submit" name="submit" value="Add">
                                    </form>
                                <?php
                            }
                        } elseif(isset($_GET['edit'])) {
                            $who = $_GET['who']; // ID of who to edit / fuck up.
                            if($_POST['submit']) {
                                ...
                                $user = $_POST['username'];
                                $pass = $_POST['password'];
                                mysql_query("UPDATE `supermod` SET `username`='".$user."',`password`='".$pass."' WHERE `id`='".$who."'");
                                echo 'Super Mod '.$user.' updated.';
                            } else {
                                $query = mysql_query('SELECT * FROM supermods WHERE `id`="'.$who.'"');
                                while($row = mysql_fetch_assoc($query)) {
                                    echo '<form action="?edit=do&yesedit=yes&who='.$who.'" method="POST">';
                            ...
                            if(isset($_GET['yesremove'])) {
                                $who = $_GET['who'];
                                echo 'Super moderator removed';
                                mysql_query("DELETE FROM `supermod` WHERE `id`='".$who."'");
                            } else {
                                ?>

                                <table border="1px solid"><tr><td>Username</td><td>Actions</td></tr>
                                <?php
                                $query = mysql_query('SELECT * FROM supermod');
                                while($row = mysql_fetch_assoc($query)) {
                                    $user = $row['username'];
                                    ?><tr>
                                        <td>
                                        ...
                                        <td><?php echo '<a href="?remove=do&yesremove=yes&who='.$row['id'].'"><button>Remove</button></a> <a href="?edit=do&yesedit=yes&who='.$row['id'].'"><button>Edit</button></a>'; ?></td>
                                      </tr>
                                    <?php
                                }?></table><?php
                            }
                            ...
                            ?>
                                <a href="?add=do"><button>Add Super Mods</button></a>
                                <a href="?remove=do"><button>View Super Mods</button></a>
                            <?php
                        }
                    ?>
                </div>
