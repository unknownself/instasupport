                        ...
                        if(isset($action)) {
                            if($action=='edit') {
                                if($_POST['submit']) {
                                    ...
                                    $name = $_POST['name'];
                                    $msg = $_POST['msg'];
                                    mysql_query("UPDATE `presets` SET `name`='".$name."', `message`='".$msg."' WHERE `id`='".$id."'");
                                    echo 'Updated preset. <a href="canned.php">Back</a>';
                                } else {
                                    $query = mysql_query('SELECT * FROM presets WHERE `id`="'.$_GET['id'].'"');
                                    while($row = mysql_fetch_assoc($query)) {
                                        $name = $row['name'];
                                        $msg = $row['message'];
                                    }
                                    ?>
                                        <form action="canned.php?act=edit&id=<?php echo $_GET['id'];?>" method="POST">
                                            User-friendly name: <input type='text' name='name' value='<?php echo $name; ?>'><br>
                                            Message<br>
                                            <textarea style='margin: 0px; width: 465px; height: 52px;' name='msg'><?php echo htmlspecialchars($msg); ?></textarea>
                                            <br>
                                            <input type='submit' name='submit' value='Update'> <small>or</small> <a href='canned.php'>Cancel changes</a>
                                        </form>
                                    <?php
                                }
                            } elseif($action=='remove') {
                                $id = $_GET['id'];
                                mysql_query('DELETE FROM `presets` WHERE `id`="'.$id.'"');
                                echo 'Preset/canned reply removed. <a href="canned.php">Back</a>';
                            } elseif($action=='add') {
                                if($_POST['submit']) {
                                    $id = $_GET['id'];
                                    $name = $_POST['name'];
                                    $msg = $_POST['msg'];
                                    //mysql_query("UPDATE `presets` SET `name`='".$name."', `message`='".$msg."' WHERE `id`='".$id."'");
                                    mysql_query("INSERT INTO `presets`(`id`, `name`, `message`) VALUES (NULL, '".$name."', '".$msg."')");
                                    echo 'Added preset. <a href="canned.php">Back</a>';
                                } else {
                                    ?>
                                        <form action="canned.php?act=add" method="POST">
                                            User-friendly Name: <input type='text' name='name' value=''><br>
                                            Message<br>
                                            <textarea style="margin: 0px; width: 465px; height: 52px;" name="msg"></textarea><br>
                                            <input type='submit' name='submit' value='Add preset/canned reply'>
                                        </form>
                                    <?php
                                }
                            } else {
                                echo 'I do not understand the request.';
                                exit;
                            }
                        } else {
                            ?>
                                <a href="?act=add">Add Presets</a> <a href="#">View Presets</a><br>

                            <?php
                            $query = mysql_query('SELECT * FROM presets');
                            while($row = mysql_fetch_assoc($query)) {
                                $name = $row['name'];
                                echo '<a href="?act=edit&id='.$row['id'].'">'.$row['name'].' | Edit</a> <a href="?act=remove&id='.$row['id'].'">| Remove</a><br>';
                            }
                        }
                    ?>
                </div>
