		...
		<div class="login-container">
			...
			if(!isset($_SESSION['op_name']))
			{
				if($_POST['submit'])
				{
					...
					$query = mysql_query("SELECT * FROM operators WHERE `op_username`='".$user."'");
					while($row = mysql_fetch_assoc($query))
					{
						$dbuser = $row['op_username'];
						$dbpass = $row['op_password'];
						if($user==$dbuser&&$pass==$dbpass)
						{
							$_SESSION['op_name']=$row['op_username'];
							$_SESSION['op_host']=$row['op_belongsto'];
							echo "<a href='operator.php'>Logged in. Click here to continue</a>";
						}
					}
					/*while($row = mysql_fetch_assoc($query))
					{
				...
				<form method="post" action="index.php">
					<fieldset>
						<div class="control-group">
							...
							<div class="controls">
								<input id="icon" type="text" placeholder="Your username" name="user">
							</div>
						</div>
						<div class="control-group">
