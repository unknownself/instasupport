	...
	<body>
		...
		<script>
			...
			function notifycheck() {
				$.ajax({
					type: "POST",
					url: "../inc/ajax_check_transfers.php",
					data: { 'me': myName },
					success: function(data)
					{
						$("#transfertab").html(data);
					}
				});
			}
			...
			function transferSuccess(chatid) {
				// we're handling the request. Start up the request functions
				$.ajax({
					type: "POST",
					url: "../inc/ajax_opfunc_tellsystem2.php",
					data: { 'handling': myName, 'chatId': chatid },
					success: function(data) 
					{
						window.location.replace("operator2.php?action=chat&chatid=" + chatid + "&name=" + myName + "&securehash=" + data);
			...
			setInterval(function(){checkrequests()},1500);
			setInterval(function(){notifycheck()},1500);
		</script>
