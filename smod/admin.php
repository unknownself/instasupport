...
if(!isset($_SESSION['smod_name'])) { exit('Security! Arrest this man!'); }
error_reporting(0);
?>
<!DOCTYPE html>
...
<header class="navbar">
    ...
    <div class="navbar-inner">
        ...
        <div class="nav-collapse">
            ...
            <nav>
                <ul role="navigation">
                    <li><a href="admin.php" class="active"><span class="awe-leaf"></span>SMOD</a></li>
...
<div class="alert alert-info fade in">
    ...
    <strong>Awesome!</strong> Super mods just got better!
</div>
<!-- /Full page alert -->
...
<div class="sidebar">
    ...
    <figure class="user-profile">
        ...
        <figcaption>
            <a href="#"><?php echo $_SESSION['smod_name']; ?></a>
            <em>super moderator <3</em>
    ...
    <section class="sidebar-box sidebar-dark">
        ...
        <p>"Listen to the damn admins"</p>
        <p class="separator">"Learn from yesterday, live for today, hope for tomorrow. The important thing is not to stop questioning"</p>
...
<article class="page-header">
    <h1>Welcome, <?php echo $_SESSION['smod_name']; ?>!</h1>
    ...
    <article class="span12 data-block nested">
        ...
        <div class="data-container">
            ...
            <section>
                ...
                <div id="sec2">
                    <h1>Welcome Supermods</h1>
                    I want to just remind you that this panel is restricted to super moderators only.
                    <br><br>
                    <?php
                    // DEBUG echo $_SESSION['adm_host'];
                    $act = htmlentities(strip_tags($_GET['action']));
                    if($act=="add_operator")
                    {
                        error_reporting(0);
                        if($_POST['submit'])
                        {
                            $rank = $_POST['rank'];
                            mysql_query("INSERT INTO `operators` (`id`, `op_username`, `op_password`, `op_status`, `op_ipaddr`, `op_belongsto`, `rank`) VALUES (NULL, '".$_POST['username']."', '".$_POST['password']."', '1', '".$_SERVER['REMOTE_ADDR']."', '".$_SESSION['smod_host']."', '".$rank."');");
                            mysql_query("INSERT INTO `ranks`(`who`, `rank`) VALUES ('".$_POST['username']."','".$rank."')");
                            ?>
                            <div class="alert"><span><b>Message</b></span> Successfully added operator</div>
                            <?php
                        }
                        else
                        {
                            ?>
                            <form action="admin.php?action=add_operator" method="POST">
                                Username: <input type="text" name="username" value="" placeholder="op_<?php echo $_SESSION['smod_host']; ?>_username" /><br>
                                Password: <input type="password" name="password" value="" placeholder="" /><br>
                                <select name="rank">
                                    <option value="Tech Mod">Tech Mod</option>
                                    <option value="Forum Mod">Forum Mod</option>
                                </select><br>
                    ...
                    if($act=="remove_operator")
                    {
                        if($_GET['delete']!="")
                        {
                            $hid = $_SESSION['smod_host'];
                            $query = mysql_query("SELECT * FROM operators WHERE `op_username`='".$_GET['delete']."'");
                            while($row = mysql_fetch_assoc($query))
                            {
                                $thehost = $row['op_belongsto'];
                            }
                            if($thehost==$hid)
                            {
                                mysql_query("DELETE FROM `operators` WHERE `op_username`='".$_GET['delete']."'");
                                echo "Deleted";
                            }
                        ...
                        else
                        {
                            ...
                            <table border="none" width="100%">
                                ...
                                <?php
                                $query = mysql_query("SELECT * FROM operators WHERE `op_belongsto`='".$_SESSION['smod_host']."'");
                                while($row = mysql_fetch_assoc($query))
                                {
                                    ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><a href="?action=remove_operator&delete=<?php echo $row['op_username']; ?>">Remove</a></td>
                                    </tr>
                                    <?php
                                }
                                ?>
                    ...
                    if($act=="")
                    {
                        $query = mysql_query("SELECT * FROM hosts WHERE `id`='".$_SESSION['smod_host']."'");
                        while($row = mysql_fetch_assoc($query))
                        {
                            ?>
                            <table border="none" width="100%">
                                ...
                                <tr>
                                    <td><?php echo $_SESSION['smod_host']; ?></td>
                                    <td><?php echo $row['hostname']; ?></td>
                                    <td><a style="color:black;" href="?action=add_operator"><button>Add Operators</button></a> <a style="color:black;" href="?action=remove_operator"><button>Remove Operator</button></a> <a style="color:black;" href="viewops.php"><button>View/edit Operators</button></a> <a style="color:black;" href="whos.php"><button>Whos chatting?</button></a></td>
                                </tr>
                            </table>
                            <?php
                        }
        ...
        <div class="data-container">
            ...
            <section>
                ...
                <div id="sec2">
                    ...
                    <small>Bo2rocks created this.</small>
                </div>
