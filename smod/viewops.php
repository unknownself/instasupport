...
<head>
    <meta charset="utf-8">
    <title>Super Moderator Panel</title>
...
<ul class="breadcrumb">
    ...
    <li><a href="#">Super Moderator</a></li>
                    ...
                    // DEBUG echo $_SESSION['adm_host'];
                        if($_GET['a']=='') {
                            ...
                            while($row =mysql_fetch_assoc($query)) {
                                echo 'OPERATOR: <a href="?a=edit&who='.$row['op_username'].'">'.$row['op_username'].'</a><br>';
                            }
                        } else {
                            $a= $_GET['a'];
                            if($a=='edit') {
                                if(empty($_GET['who'])) {
                                   exit('Who should I edit? You didnt tell me.');
                                }
                                if($_POST['submit']) {
                                    $pass = $_POST['pass'];
                                    $rank = $_POST['rank'];
                                    $query = mysql_query('SELECT * FROM operators WHERE `op_username`="'.$_GET['who'].'"');
                                    while($row = mysql_fetch_assoc($query)) {
                                        $passdb = $row['pass'];
                                        $rankdb = $row['rank'];
                                    }
                                    if($pass!=$passdb) {
                                        mysql_query('UPDATE `operators` SET `op_password`="'.$pass.'" WHERE `op_username`="'.$_GET['who'].'"');
                                    }
                                    if($rank!=$rankdb) {
                                        mysql_query('UPDATE `operators` SET `rank`="'.$rank.'" WHERE `op_username`="'.$_GET['who'].'"');
                                        mysql_query('UPDATE `ranks` SET `rank`="'.$rank.'" WHERE `who`="'.$_GET['who'].'"');
                                    }
                                    echo 'Updated<br>';
                                }
                                // obtain operator information - step 1
                                $query = mysql_query('SELECT * FROM operators WHERE `op_username`="'.$_GET['who'].'"');
                                while($row = mysql_fetch_assoc($query)) {
                                    echo '<form action="?a=edit&who='.$_GET['who'].'" METHOD="post">';
                                        echo 'Username: <input type="text" name="user" value="'.$row['op_username'].'" disabled><br>';
                                        echo 'Password: <input type="password" name="pass" value="'.$row['op_password'].'"><br>';
                                        ?>
                                            <!-- ranks -->
