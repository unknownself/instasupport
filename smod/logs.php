...
<!--[if gt IE 9]><!--> <html class="no-js" lang="en"> <!--<![endif]-->
						...
						<div class="data-container">
							...
							<section>
								...
								<div id="sec2">
									<?php
										if($_POST['submit']) {
											$vlf = htmlentities(strip_tags($_POST['vlf']));
											$logs_dir = 'op_logs';
											if($vlf=="all") {
												echo '<b>Regular logs(action logs)</b><br>';
												echo '<table border="1">';
												?> <tr>
    <th scope="col">#</th>
    <th scope="col">WHO</th>
    <th scope="col">LOG MESSAGE</th>
												...
												$query = mysql_query("SELECT * FROM logs WHERE `type`='regular' ORDER BY ID DESC");
    ...
    <th scope="col">ACTIONS</th>
												...
												$query = mysql_query("SELECT * FROM logs WHERE `type`='chat' ORDER BY ID DESC");
												while($row = mysql_fetch_assoc($query)) {
													echo '<tr>';
													echo '<td>'.$row['id'].'</td>';
													echo '<td>'.$row['who'].'</td>';
													echo '<td><a href="open.php?id='.$row['message'].'">View</a></td>';
													echo '</tr>';
												}
												echo '</table>';
											} else {
    ...
    <th scope="col">LOG MESSAGE</th>
												...
												$query = mysql_query("SELECT * FROM logs WHERE `who`='".$vlf."' AND `type`='regular' ORDER BY ID DESC");
												while($row = mysql_fetch_assoc($query)) {
													echo '<tr>';
													echo '<td>'.$row['id'].'</td>';
													echo '<td>'.$row['who'].'</td>';
    ...
    <th scope="col">ACTIONS</th>
												...
												$query = mysql_query("SELECT * FROM logs WHERE `who`='".$vlf."' AND `type`='chat' ORDER BY ID DESC");
												while($row = mysql_fetch_assoc($query)) {
													echo '<tr>';
													echo '<td>'.$row['id'].'</td>';
													echo '<td>'.$row['who'].'</td>';
											...
											?>
												<form action="logs.php" method="POST">
													Actions<br>
													View logs for: <input type='text' name='vlf' value='all'><br>
